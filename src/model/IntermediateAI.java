  ...
  public Point desiredMove(TicTacToeGame theGame) {
    ...
    if(theGame.maxMovesRemaining() == 8) {
      ...
      if(theGame.available(1, 1)) {
        ...
        Random initialRan = new Random();
        int initialRanNum = initialRan.nextInt(2);

        if(initialRanNum == 0) {
          initialRanNum = initialRan.nextInt(2);
      ...
      //check for two X in diagonal to prevent opponent win
      // Check Diagonal from upper left to lower right
      sum = 0;
      for (int r = 0; r < size; r++) {
        if (look[r][r] == 'X')
